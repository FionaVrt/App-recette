...
import { Plus, Trash2, ChefHat, ExternalLink, Loader, BookOpen, Clock, Share2, Copy, Download, Search, Star, Moon, Sun, Edit2, X } from 'lucide-react';
      ...
      if (keys && keys.keys) {
        ...
        for (const key of keys.keys) {
          try {
            ...
            if (result) {
              loadedRecipes.push(JSON.parse(result.value));
            }
          } catch (err) {
            console.log('Erreur de lecture:', err);
          }
        }
        setRecipes(loadedRecipes.sort((a, b) => b.timestamp - a.timestamp));
      }
    } catch (error) {
...
Réponds UNIQUEMENT avec un JSON dans ce format exact (sans markdown, sans backticks) :
{
  ...
  const toggleFavorite = async (recipe) => {
    ...
    await window.storage.set(`recipe:${recipe.id}`, JSON.stringify(updatedRecipe));
    await loadRecipes();
  ...
  const importRecipe = async () => {
    try {
      ...
      if (recipe.title && recipe.ingredients) {
        ...
        await window.storage.set(`recipe:${newRecipe.id}`, JSON.stringify(newRecipe));
        await loadRecipes();
